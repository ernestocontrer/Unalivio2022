...
const urls = {
  cryptocompare: (fsym: string = "usd", tsym: string = "dash") =>
    `https://min-api.cryptocompare.com/data/pricemulti?fsyms=${fsym}&tsyms=${tsym}`,
  yadio: (fsym: string = "usd") => `https://api.yadio.io/rate/${fsym}`,
  bitso: (fsym: string = "btc", tsym: string = "mxn") =>
    `https://api.bitso.com/v3/ticker/?book=${fsym}_${tsym}`,
};
...
export const main = (db: FirebaseFirestore.Firestore) =>
  functions
    ...
    .onRun(async (context: functions.EventContext) => {
      ...
      const btcmxn: number = +(await fetch(options.price.bitso("btc", "mxn")))
        .data["payload"]["last"];

      const btcves: number = +(await fetch(options.price.yadio("btc", "ves")))
        .data["rate"];
      ...
      const commission = 0.1; // biyuyo 6%
      const referral = 0.02;
      const utility = 0.05;
      const vesmxn =
        (btcves / btcmxn) * (1 - (commission + referral + utility));
