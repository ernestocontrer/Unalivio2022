...
const computePrice = (
	...
	rate: number,
	discount: number = 0
): number => Math.ceil((amount / rate - discount + Number.EPSILON) * 100) / 100;

export const generate = (db: FirebaseFirestore.Firestore, stripe: Stripe) =>
...
export const verify = (db: FirebaseFirestore.Firestore, stripe: Stripe) =>
	functions.https.onRequest(
		verifyApp(stripe, {
			...
			'charge.succeeded': async (charge: Stripe.Charge) => {
				...
				const snap = await db
					.collection('orders')
					.where('intent', '==', charge.payment_intent)
					.get();
